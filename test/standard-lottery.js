...
describe("StandardLottery", () => {
  ...
  it("start/close lottery", async () => {
    ...
    await expect(
      this.standardLottery.connect(operator).closeLottery(lotteryId)
    ...
    const { status } = await this.standardLottery.viewLottery(lotteryId);
  ...
  it("buy tickets", async () => {
    ...
    await this.dehubToken
      .connect(alpha)
      .approve(this.standardLottery.address, DEHUB_PRICE * 2);
    ...
    await this.dehubToken
      .connect(beta)
      .approve(this.standardLottery.address, DEHUB_PRICE * 6);
    await this.standardLottery.connect(beta).buyTickets(
      lotteryId,
    ...
    await this.dehubToken
      .connect(gamma)
      .approve(this.standardLottery.address, DEHUB_PRICE * 10);
    await this.standardLottery.connect(gamma).buyTickets(
      lotteryId,
  ...
  describe("play game", async () => {
    ...
    it("transfer to", async () => {
      ...
      await this.standardLottery.transferTo(operator.address, DEHUB_PRICE);
      expect(
