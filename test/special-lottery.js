...
describe("SpecialLottery", () => {
  ...
  it("start/close lottery", async () => {
    ...
    await expect(
      this.specialLottery.connect(operator).closeLottery(lotteryId)
  ...
  it("buy tickets", async () => {
    ...
    await this.specialLottery.connect(alpha).buyTickets(
      lotteryId,
      50 // purchased ticket count
    );
    await this.dehubToken
      .connect(beta)
      .approve(this.specialLottery.address, DEHUB_PRICE * 40);
    await this.specialLottery.connect(beta).buyTickets(
      lotteryId,
      40 // purchased ticket count
    );
    await this.dehubToken
      .connect(gamma)
      .approve(this.specialLottery.address, DEHUB_PRICE * 30);
    await this.specialLottery.connect(gamma).buyTickets(
      lotteryId,
    ...
    expect(
      alphaInitBalance - (await this.dehubToken.balanceOf(alpha.address))
    ).to.equal(DEHUB_PRICE * 50);
    expect(
      betaInitBalance - (await this.dehubToken.balanceOf(beta.address))
    ).to.equal(DEHUB_PRICE * 40);
    expect(
      gammaInitBalance - (await this.dehubToken.balanceOf(gamma.address))
    ...
    expect(
      (await this.dehubToken.balanceOf(this.standardLottery.address)) -
        deLottoInitBalance
    ...
    // Get ticket ids
    const userInfo = await this.specialLottery
      .connect(alpha)
      .viewUserInfoForLotteryId(alpha.address, lotteryId, 0, 100);
    await expect(
      this.specialLottery.connect(alpha).claimTickets(lotteryId, userInfo[0])
  ...
  describe("play game", async () => {
    beforeEach(async () => {
      ...
      await this.dehubToken
        .connect(alpha)
        .approve(this.specialLottery.address, DEHUB_PRICE * 2);
      await this.specialLottery.connect(alpha).buyTickets(
        lotteryId,
      ...
      await this.dehubToken
        .connect(beta)
        .approve(this.specialLottery.address, DEHUB_PRICE * 3);
      await this.specialLottery.connect(beta).buyTickets(
        lotteryId,
      ...
      await this.dehubToken
        .connect(gamma)
        .approve(this.specialLottery.address, DEHUB_PRICE * 4);
      await this.specialLottery.connect(gamma).buyTickets(
        lotteryId,
    ...
    it("pick delotto second stage", async () => {
      ...
      const alphaInitBalance = await this.dehubToken.balanceOf(alpha.address);
      const unwonPot = await this.dehubToken.balanceOf(
        this.standardLottery.address
      );
      ...
      const userInfo = await this.specialLottery
        .connect(alpha)
        .viewUserInfoForLotteryId(alpha.address, lotteryId, 0, 100);
      await this.specialLottery
        .connect(alpha)
        .claimTickets(lotteryId, userInfo[0]);
      expect(
        (await this.dehubToken.balanceOf(alpha.address)) - alphaInitBalance
    ...
    it("pick degrand stage", async () => {
      ...
      const deGrandInfo = await this.specialLottery
        .connect(alpha)
        .viewDeGrandStatusForTicketIds(lotteryId);
  ...
  it("transfer to", async () => {
    ...
    await this.specialLottery.transferTo(operator.address, DEHUB_PRICE);
    expect(
      (await this.dehubToken.balanceOf(operator.address)) - initBalance
