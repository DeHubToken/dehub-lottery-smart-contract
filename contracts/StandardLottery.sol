...
contract StandardLottery is Ownable, ReentrancyGuard, IDeHubRandConsumer {
  ...
  function buyTickets(
    ...
    // According to bundle rule, return
  ...
  function burnUnclaimed(uint256 _lotteryId) external onlyOperator {
    ...
    if (remain > 0) {
      dehubToken.safeTransferFrom(address(msg.sender), deadAddress, remain);
    }
  ...
  function setBundleRule(
    ...
    require(bundleRules.length < MAX_BUNDLE_RULES, "Maximum bundle rules");
  ...
  function _viewBundleRule(uint256 _purchasedTickets) internal view returns (uint256) {
    ...
    uint256 freeTickets = bundleRules[0].freeTickets;
    for (uint256 i = 1; i < bundleRules.length; i++) {
      if (bundleRules[i].purchasedTickets <= _purchasedTickets) {
        freeTickets = bundleRules[i].freeTickets;
      }
    }
    return freeTickets;
  }
   ...
   * @dev Callable by users
   */
  function viewBundleRule(uint256 _purchasedTickets) external view returns (uint256) {
    return _viewBundleRule(_purchasedTickets);
  }
